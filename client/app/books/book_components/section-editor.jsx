...
export default function SectionEditor({ book, section, onBack, onSave }) {
  ...
  function removeEmptyPages(pagesArr) {
    ...
    return removed.length > 0
      ? removed
      : [pagesArr[0] || { title: "", content: "", editorContent: null }];
  }

  function extractChaptersFromDrafts(drafts) {
    //returns even existing chapters with new ids
    let chapterObjs = [];
    ...
    chapters.forEach((chapter) => {
      const wordCount = (
        removeEmptyPages(chapter.pages) || chapter.pages
      ).reduce((count, page) => {
        if (page.content) {
  ...
  const handleSave = () => {
    ...
    if (updatedBook[sectionTypeKey]) {
      updatedBook[sectionTypeKey] = updatedBook[sectionTypeKey].map((item) =>
        item.id === section.id ? updatedSection : item
      );
    } else {
      console.error(`Unknown section type or array not found: ${section.type}`);
