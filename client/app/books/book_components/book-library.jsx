...
export default function BookLibrary({ books, onOpenBook, onUpdateBook, onNewProject }) {
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-6">
        <div>
          ...
          <p className="text-muted">Manage your active writing projects</p>
        </div>
        <Button className="btn-primary font-medium mobile-full" onClick={() => onNewProject()}>
          ...
          >
            <CardHeader className="pb-3 mobile-p">
              <div className="flex justify-between items-start">
                ...
                <Badge className="badge-muted">{book.status}</Badge>
              </div>
              <div className="flex flex-wrap gap-2 mt-2">
                {book.tags.map((tag) => (
                  <Badge className="badge-accent text-xs" key={tag}>
                    {tag}
                  </Badge>
                ))}
              </div>
            ...
            <CardContent className="mobile-p">
              ...
              <Button className="btn-outline w-full mt-4" onClick={() => onOpenBook(book)}>
