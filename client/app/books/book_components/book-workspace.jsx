...
import { ArrowLeft, Plus, Edit, Search, Filter, FileText, Users, ImageIcon, Lightbulb, Eye } from "lucide-react"

export default function BookWorkspace({ book, onBack, onEditSection, onUpdateBook }) {
  const [searchTerm, setSearchTerm] = useState("")
  const [tagFilter, setTagFilter] = useState("All")
  const [activeTab, setActiveTab] = useState("drafts")

  const allTags = [...new Set([...book.drafts.map((d) => d.tag), ...book.notes.map((n) => n.tag)])].filter(Boolean)
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "editing":
        return { backgroundColor: "var(--accent-secondary)", color: "var(--accent-foreground)" }
      default:
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
        <TabsList className="grid grid-cols-4 p-1 rounded-lg bg-muted overflow-x-auto">
          <TabsTrigger
            ...
            className={`flex items-center gap-2 transition-all ${
              activeTab === "drafts" ? "tab-active" : "tab-inactive"
            }`}
          >
          ...
          <TabsTrigger
            value="notes"
            className={`flex items-center gap-2 transition-all ${
              activeTab === "notes" ? "tab-active" : "tab-inactive"
            }`}
          >
          ...
          <TabsTrigger
            value="characters"
            className={`flex items-center gap-2 transition-all ${
              activeTab === "characters" ? "tab-active" : "tab-inactive"
            }`}
          >
            <Users className="w-4 h-4" />
            <span className="hidden sm:inline">Characters</span>
            <span className="sm:hidden">{book.characters.length}</span>
          </TabsTrigger>
          <TabsTrigger
            value="assets"
            className={`flex items-center gap-2 transition-all ${
              activeTab === "assets" ? "tab-active" : "tab-inactive"
            }`}
          >
        ...
        <TabsContent value="drafts">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 mb-4">
            <h3 className="text-lg font-semibold">Draft Scenes & Chapters</h3>
            <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredDrafts.length === 0 ? (
              <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12">
                ...
                <p className="text-muted">No drafts found. Create your first draft!</p>
              </div>
            ) : (
              filteredDrafts.map((draft) => (
                <Card key={draft.id} className="card hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  <CardHeader className="pb-3 mobile-p">
                    <div className="flex justify-between items-start">
                      <CardTitle className="text-base">{draft.title}</CardTitle>
                      <Badge className="badge-accent" variant="outline">
                        {draft.status}
                      </Badge>
                    </div>
                    <Badge className="badge-muted text-xs mt-2">{draft.tag}</Badge>
                  </CardHeader>
                  <CardContent className="mobile-p">
                    <p className="text-sm line-clamp-3 mb-3 text-muted">{draft.content}</p>
                    <Button
                      variant="outline"
                      size="sm"
        ...
        <TabsContent value="notes">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredNotes.length === 0 ? (
              <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12">
                <Lightbulb className="w-12 h-12 mx-auto mb-4" style={{ color: "var(--muted-foreground)" }} />
                <p className="text-muted">No notes found. Add your first research note or idea!</p>
              </div>
            ) : (
              filteredNotes.map((note) => (
                <Card key={note.id} className="card hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  <CardHeader className="pb-3 mobile-p">
                    <CardTitle className="text-base">{note.title}</CardTitle>
                    <Badge className="badge-muted text-xs mt-2">{note.tag}</Badge>
                  </CardHeader>
                  <CardContent className="mobile-p">
                    <p className="text-sm line-clamp-3 mb-3 text-muted">{note.content}</p>
                    <Button
                      variant="outline"
                      size="sm"
        ...
        <TabsContent value="characters">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 mb-4">
            <h3 className="text-lg font-semibold">Character Profiles</h3>
            <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {book.characters.length === 0 ? (
              <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12">
                ...
                <p className="text-muted">No characters found. Create your first character profile!</p>
              </div>
            ) : (
              book.characters.map((character) => (
                ...
                >
                  <CardHeader className="pb-3 mobile-p">
                    ...
                    <Badge className="badge-muted text-xs mt-2">{character.role}</Badge>
                  </CardHeader>
                  <CardContent className="mobile-p">
                    <p className="text-sm line-clamp-3 mb-3 text-muted">{character.notes}</p>
                    <Button
                      variant="outline"
                      size="sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {book.assets.length === 0 ? (
              <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-12">
                ...
                <p className="text-muted">No assets found. Add your first reference or asset!</p>
              </div>
            ) : (
              book.assets.map((asset) => (
                <Card key={asset.id} className="card hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  <CardHeader className="pb-3 mobile-p">
                    <CardTitle className="text-base">{asset.name}</CardTitle>
                    <Badge className="badge-muted text-xs mt-2">{asset.type}</Badge>
                  </CardHeader>
                  <CardContent className="mobile-p">
                    <p className="text-sm line-clamp-3 mb-3 text-muted">{asset.description}</p>
                    <Button
                      variant="outline"
                      size="sm"
